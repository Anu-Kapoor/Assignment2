...
import { setContributionFileUploadDialogOpen } from '../../../slices/layoutSlice';

import {

  getCurrentSubmissionBatchId,

  getSubmissionBatchById,

} from 'src/selectors/submissionBatchesSelectors';

import Skeleton from '@material-ui/lab/Skeleton/Skeleton';

import { hasTemplatesFetched } from 'src/selectors';

import { SubmissionBatchStatus } from 'src/enums';
...
const UploadButton: React.FC = () => {
  ...
  const currentBatchId = useSelector(getCurrentSubmissionBatchId);

  const submissionBatch = useSelector(

    getSubmissionBatchById(currentBatchId as string)

  );

  const isAvailable = useSelector(hasTemplatesFetched) && submissionBatch;



  const handleOpen = useCallback(() => {

    dispatch(setContributionFileUploadDialogOpen(true));
  ...
  const isBatchClosed =
    ...
    submissionBatch && submissionBatch.status === SubmissionBatchStatus.Closed;
  ...
  if (!isAvailable || !ready) {
    ...
    return <Skeleton variant="text" width="100px" height="32px" />;
